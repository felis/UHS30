...
readmore:
        do {
                ...
                if(x >= 64) {
                        ...
                        if(rcode && rcode != UHS_HOST_ERROR_NAK) return;
                        if(x > 0) {
                                rd_head += x;
                        }
...
#if defined(LOAD_UHS_CDC_ACM_FTDI)
        if(wr_head != wr_tail && (txe || adaptor != UHS_USB_ACM_FTDI)) {
#else
        if(wr_head != wr_tail) {
#endif
                if(wr_tail < wr_head) {
...
uint8_t MY_ACM::OnStart(void) {
        ...
        if(adaptor == UHS_USB_ACM_FTDI) {
                ...
                if(rcode) {
                        printf_P(PSTR("FTDI: Set latency timer error %x\r\n"), rcode);
                        return rcode;
                }
        ...
        if(Acm->isReady()) {
                ...
                if(rcvd > 0) {
                        ...
                        if(chunk_size > 0) {
                                ...
                                USB_HOST_SERIAL.readBytes((char *)chunk, rcvd);
                                // Tell USB to send it
                ...
                if(chunk_size > 0 && rcvd > 0) {
                        ...
                        USB_HOST_SERIAL.write((char *)chunk, chunk_size);
